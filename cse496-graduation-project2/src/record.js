(async () => {
    ...
    function toBinary(data) {
        // Create a new Float32Array from the data array
    ...
    function setUpRecording() {
        ...
        recorder.onaudioprocess = function (e) {
            ...
            // Send the 1-second audio to classification server
            if ((recordingLength / sampleRate) > dataSent) {
                offset = recordingLength / bufferSize;

                /*console.log("Recording lenght: " + recordingLength);
                console.log("Data sent: " + dataSent);
                console.log("Data index: " + dataIndex);
                console.log("Offset: " + offset);*/

                leftChunk = leftChannel.slice(dataIndex, offset);
                rightChunk = rightChannel.slice(dataIndex, offset);

                /*console.log("Left chunk lenght: " + leftChunk.length*bufferSize);
                console.log("Right chunk lenght: " + rightChunk.length*bufferSize);

                console.log(leftChunk)
                console.log(rightChunk)*/

                leftChunk = mergeBuffers(leftChunk, leftChunk.length*bufferSize);
                rightChunk = mergeBuffers(rightChunk, rightChunk.length*bufferSize);

                /*console.log(leftChunk)
                console.log(rightChunk)*/

                // Convert the data to a binary array
                let data = interleave(leftChunk, rightChunk);
                let binary = toBinary(data);

                // Send the binary data to the server over the WebSocket connection
                ws.send(binary);
        ...
        // Set up an event listener to handle messages from the server
